...
def test_filter_paragraph_range() -> None:
    result = parse_gii_xml(FIXTURE_BGB_AUSZUG)
    filtered = filter_normen(result, paragraph_range=("535", "580a"))
    assert "535" in filtered
    assert "536" in filtered
    assert "536a" in filtered
    assert "600" not in filtered  # außerhalb der Range


def test_filter_explicit_list() -> None:
    result = parse_gii_xml(FIXTURE_BGB_AUSZUG)
    filtered = filter_normen(result, paragraphen=["535", "600"])
    assert set(filtered.keys()) == {"535", "600"}
